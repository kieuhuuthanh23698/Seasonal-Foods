...
public class DBHelper {
    String DATABASE_NAME = "QLSV.sqlite";
    ...
    private void processSQLite() {
        ...
        if(!dbFile.exists()){
            try{
                ...
                Toast.makeText(context, "Copy successful !!!",
    ...
    private void CopyDatabaseFromAsset() {
        try{
            ...
            File file = new
                    File(context.getApplicationInfo().dataDir + DB_PATH_SUFFIX);
            if(!file.exists()){
                file.mkdir();
            }
            ...
            int length;
            while ((length = databaseInputStream.read(buffer))
                    > 0){
    ...
    public void insertStudent(Student student){
        ...
        if(db.insert("sinhvien",null,contentValues) > 0){
            Toast.makeText(context, "Insert successful",
                    Toast.LENGTH_SHORT).show();
        }
    }
    public void updateStudent(Student student){
        ...
        contentValues.put("address",student.getAddress());
        contentValues.put("gender",student.getGender());
        if(db.update("sinhvien",contentValues,"id =" + student.getId(),null) > 0){
    ...
    public void deleteStudent(Student student){
        db = context.openOrCreateDatabase(DATABASE_NAME,context.MODE_PRIVATE,null);
        if(db.delete("sinhvien","id =" + student.getId(),null) > 0){
